...
void salvaLivro( Livro* livro, char* arquivo){
    FILE* arq = fopen(arquivo, "ab, ccs=UTF-8"); //o caracter "b" indica que o arquivo deve ser aberto em modo binário
    if(arq == NULL){
    ...
    //fwrite("\0", sizeof(char), 1, arq); // adiciona um caractere nulo ao final do campo "titulo"
...
void lerbiblioteca( Livro* lista, char* arquivo, int n){
    FILE* arq = fopen(arquivo, "rb, ccs=UTF-8"); //leitura no modo binário
    if(arq == NULL){
    ...
    while(ftell(arq) < size && i < n){
        ...
        long int byte_offset = ftell(arq);

        lista[i].byteOffset = byte_offset;
        ...
        lista[i].autor[autor_tamanho] = '\0'; // adiciona um caractere nulo ao final do campo "autor"
        // Autor
